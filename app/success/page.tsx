...
export default function SuccessPage() {
    ...
    async function fetchData() {
      try {
        ...
        if (sess.payment_status === "paid") {
          setSession(sess);
          setStatus("paid");
          setCustomerEmail(sess.customer_email ?? sess.customer_details?.email ?? null);
  ...
  return (
    <div className="max-w-xl mx-auto mt-20 p-10 bg-black text-white rounded-2xl shadow-xl space-y-6">
      ...
      <p className="text-center">Thank you for your purchase—your payment was successful.</p>
      ...
      <div className="bg-zinc-900 rounded-lg p-4 space-y-3">
        <h2 className="font-black uppercase border-b border-zinc-700 pb-2">Order Summary</h2>
        {lineItems.map((li) => (
          <div key={li.description} className="flex justify-between">
            ...
            <p className="font-black">${((li.amount_subtotal ?? 0) / 100).toFixed(2)}</p>
          </div>
        ))}
